...
// StartAutoReload 启动自动重载（基于文件监听）
func (e *Engine) StartAutoReload() {
	if len(e.rulesPaths) == 0 {
		logger.Info("没有规则文件路径，跳过文件监听")
		return
	}
	
	// 创建文件监听器
	w, err := watcher.NewWatcher(e.rulesPaths, func(rules []config.Rule) error {
		e.mutex.Lock()
		e.rules = rules
		e.mutex.Unlock()
		
		logger.Info("规则已通过文件监听器更新")
		return nil
	})
